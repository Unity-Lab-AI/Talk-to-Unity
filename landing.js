...
const dependencyChecks = [
    {
        ...
        check: () =>
            ...
            /^localhost$|^127(?:\.\d{,3}){3}$|^\[::1\]$/.test(window.location.hostname)
    },
    {
        id: 'speech-recognition',
        label: 'Web Speech Recognition API',
        friendlyName: 'speech listening light',
        check: () => Boolean(SpeechRecognition || window.MediaRecorder)
    },
    {
        id: 'speech-synthesis',
...
function showRecheckInProgress() {
    ...
    updateLaunchButtonState({ allMet: false, missing: [] });
    if (launchButton) {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    launchButton?.addEventListener('click', () => {
        ...
        if (allMet) {
            ...
            document.cookie = `checks-passed=true; expires=${expiryDate.toUTCString()}; path=/`;
    ...
    if (announce) {
        if (allMet) {
            ...
            const message = summary
                ? `Some features may be limited: ${summary}.`
